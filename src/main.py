...
API compatibility:
  Accepts optional --input (plan image) and --outdir (artifact dir) so the FastAPI
  wrapper can pass them. The current pipeline does not use the image directly, but
  we accept --input to avoid argparse errors. Artifacts are produced under data/output/
  and then copied to --outdir at the end.
"""
...
def main():
    ...
    # --- New flags used by the API wrapper ---
    ...
    # Where the legacy scripts read/write
    DATA_OUTPUT = Path("data/output")
    DATA_OUTPUT.mkdir(parents=True, exist_ok=True)

    # Where the API expects artifacts
    OUTDIR = Path(args.outdir)
    OUTDIR.mkdir(parents=True, exist_ok=True)

    if args.input_image:
        # Currently unused by this pipeline, but accepted for API compatibility.
    ...
    # ---- Copy final artifacts into OUTDIR for the API to serve ----
    ...
    print("Primary outputs in data/output/:")
